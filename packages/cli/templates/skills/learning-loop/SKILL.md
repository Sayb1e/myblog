---
name: learning-loop
description: Use when working in a markdown learning workspace managed by MyBlog (files like PROGRESS.md, GOALS.md, YYYY-MM-DD/SUMMARY.md, or myblog.config.json). Triggers on "今天学什么", "今天该干什么", "下次从哪继续", "上次停在哪", "收工", "写总结", "学习进度", "G 阶段/G 编号". Use ONLY for managing that workspace; not for general coding.
---
...
myblog_read_summary # 读某天 YYYY-MM-DD/SUMMARY.md
myblog_scaffold     # 建当日总结骨架（不覆盖；只在当天确有进展时才建）
...
2. **定任务**：今日任务 = 「下次从哪继续」∩ 当前阶段 G 能力。不要因为学习目标里写了脱壳/AOSP/CVE 就提前开线；主线未闭环不并新阶段。
3. **做**：按用户确认的方案执行学习内容（这是唯一由你与用户共同推进的部分）。
4. **记**：当天确有进展时，才在 `YYYY-MM-DD/SUMMARY.md` 的 `## 这次` 里标当天碰到的 G 编号（可多个，只标真正碰到的）。当天新建的工程/脚本/APK 放进 `YYYY-MM-DD/` 子目录。
5. **收工**：先 `myblog_close`（`dryRun=true`）给用户看 diff，确认后再以 `dryRun=false` 落盘；最后 `myblog_check`。

## 硬约束

- **有进展才生成**：当天没有实际进展（没做、没学、没得到结论）时，不要建日期目录/当日总结、不要写 `PROGRESS.md`、也不要调用 `myblog_close`。只有确有进展（做了、学了、`下次从哪继续` 有变化）才创建当日总结并写回；`PROGRESS.md` 同理。
