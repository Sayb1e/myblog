...
import { openWorkspace, rootOf } from "../context.js";
  ...
  program
    ...
    .action(async (options: InitCliOptions) => {
      ...
      if (options.workspace) {
        ...
        const created = await workspace.initWorkspace();
        if (created.created.length === 0) {
          console.log("学习仓文件已存在，未覆盖。");
        } else {
          for (const file of created.created) console.log(`创建学习仓文件  ${file}`);
        }
      }
    });
}
